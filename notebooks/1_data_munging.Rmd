...
# Introduction
In this session, you will learn the basic grammar of data manipulation and visualization, and some best-practice advices. We will here focus mainly on
...
Comparisons return boolean values: `TRUE` or `FALSE` (often abbreviated and equivalent to `T` and `F`)
...
![](https://www.dropbox.com/s/13okk1po1gqae6b/pipe.jpg?dl=1)
...
![](https://www.dropbox.com/s/bmonr9ulkd9v5tf/dplyr_filter.png?dl=1)
...
![](https://www.dropbox.com/s/3vmtmr0rfq7fe5p/un.jpeg?dl=1)
...
votes %>%
  summarize(total = n(),
            percent_yes = mean(vote == 1) %>% round(2)) 
```
...
2: or we want to replace a certain character string... eg. because it is just too long... The tidyverse library `stringr` can be very helpful there.
...
An [kaggle notebook exercise](https://www.kaggle.com/danielhain/sds-2018-m1-2-data-munging-exercise-3) is waiting for you:
...
* Use aestetics such as `col`, `fill`, `size`, `shape`
* Use `facet_wrap()`
